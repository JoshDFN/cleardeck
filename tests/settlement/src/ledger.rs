...
//!
//! Only the parts this harness needs: an init payload with ICRC-2 enabled and the
//! real fee, `icrc1_transfer`, `icrc2_approve`, `icrc1_balance_of`.
//!
//! Shapes taken from `rs/ledger_suite/icp/ledger.did` at the pinned commit in
//! [`crate::wasms`]. The settlement oracle needs a real ledger only so the chips
//! it is auditing came from real money: `buy_in` spends escrow, escrow comes from
//! an ICRC-2 pull, and that pull must be the production path.
...
/// An init payload that funds `initial`, enables ICRC-2 (required by
/// `deposit`), and charges the real 10_000 e8s fee.
pub fn init_payload(
    minter: Principal,
    ...
    let args = InitArgs {
        ...
        // No archiving: an archived block would change what the canister can see.
